...
public class TutorialLeapMotionGesture : MonoBehaviour
{
	...
	public GameObject[] LearningCanvas=new GameObject[4];
	public int CanvasNumber;
	public bool CreateClay;
	public bool Rewind;
	public bool Pause;
...
//		}
		LearningCanvas [CanvasNumber].SetActive (true);
    ...
    void Update()
    {
		...
		if (CanvasNumber == 2) {
			...
			{
				GameObject.FindWithTag ("IndexFinger").GetComponent<BoxCollider> ().enabled = true;
			}
			LearningCanvas [2].SetActive (true);
        ...
        if (LearningPause) //콘텐츠 강의 정지
        {
            if (mFrame.Hands.Count == 1) //손이 1개 일때
            {
				bool RightHand = mFrame.Hands.Frontmost.IsRight; //오른손인지 판단
		...
		if (NextSkip) //스
		{
			if (mFrame.Hands.Count == 1) //손이 1개 일때
			{
				bool LeftHand = mFrame.Hands.Frontmost.IsLeft; //왼손 판단

				if(LeftfingerCount < 3)
				{
					if (LeftHand)
					{
						//NextSkip = false;
						...
						Debug.Log ("LeftFist");
						LearningCanvas [CanvasNumber].SetActive (false);

						if (CanvasNumber == 1) {
							//LearningCanvas [0].SetActive (false);
							CanvasNumber = 2;
							...
							ReceiveFunction (6);
							//Destroy (LearningCanvas [5]);
							LearningCanvas[5].SetActive(false);
							gameObject.GetComponent<TutorialLeapMotionGesture> ().LeftLotation = true;
							gameObject.GetComponent<TutorialLeapMotionGesture> ().LearningPlay = true;
							gameObject.GetComponent<TutorialLeapMotionGesture> ().LearningPause = true;
							gameObject.GetComponent<TutorialLeapMotionGesture> ().LearningRewind = true;
						}
        ...
        if (LearningFastPlay || LearningRewind) //콘텐츠 강의 빠른 재생, 콘텐츠 강의 되감기
        {
            if (mFrame.Hands.Count == 1)
            {
                ...
                else if (LearningRewind && leftHandSwipeOut) //되감기
                {
					...
					GameObject.FindWithTag ("CLAY").GetComponent<MeshDeformer> ().springForce = 10.0f;
		...
		if (Rotation = true && Rewind == true && CreateClay == true && Pause==true) {
	...
	*/
	...
	public void ReceiveFunction(int Receive)
	{
		CanvasNumber = Receive;
		NextSkip = true;
...
// Structs and enums //
public struct TutorialEasyLeapGesture
{
	public TutorialEasyLeapGestureType Type;
	public TutorialEasyLeapGestureTypeState State;
...
public enum TutorialEasyLeapGestureType
{
    DEFAULT,
    TYPECIRCLE,
    TYPESWIPE,
...
public enum TutorialEasyLeapGestureTypeState
{
    STATEINVALID,
    STATESTART,
    STATEUPDATE,
